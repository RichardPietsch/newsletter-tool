...
export function renderEventGrid(block: EventGridBlock) {
  ...
  for (let i = 0; i < block.items.length; i += 2) {
    ...
    if (remaining === 1) {
      out += `<mj-section background-color="${styles.newsletterBackground}" padding="12px ${px(styles.eventGrid.outerPaddingX)}"><mj-column background-color="${styles.teaserBackground}" border-radius="4px">${card(block.items[i])}</mj-column></mj-section>`;
    } else {
      out += `<mj-section background-color="${styles.newsletterBackground}" padding="12px ${px(styles.eventGrid.outerPaddingX)}"><mj-column width="50%" background-color="${styles.teaserBackground}" border-radius="4px">${card(block.items[i])}</mj-column><mj-column width="50%" background-color="${styles.teaserBackground}" border-radius="4px">${card(block.items[i + 1])}</mj-column></mj-section>`;
    }
  }
  return out;
}
