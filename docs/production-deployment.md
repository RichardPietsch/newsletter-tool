...
Das Mandanten-Design wird vollständig und versioniert in `app_settings.settings` gespeichert: Standard-Header einschließlich der individuellen Einstellung für abgerundete Bildecken, Light- und Dark-Mode-Farben sowie der Standard-Footer bilden gemeinsam ein Dokument. Der Migrationslauf ergänzt fehlende Datensätze und führt ältere Dokumentformen einmalig in dieses Format über. Bereits aktuelle Dokumente werden dabei nicht aktualisiert; normale Lesezugriffe schreiben ebenfalls niemals Einstellungen zurück. Ein optimistischer JSONB-Vergleich verhindert, dass die Migration eine tatsächlich parallel vorgenommene Änderung überschreibt, ohne von der unterschiedlichen Zeitstempelpräzision in PostgreSQL und JavaScript abhängig zu sein. Kann ein Datensatz nicht migriert werden, nennt die Ausgabe Mandanten-ID, Mandantenname und die ungültigen beziehungsweise fehlenden Dokumentfelder. Das Backup prüft ausdrücklich, dass die Tabellendaten von `app_settings` im PostgreSQL-Archiv enthalten sind. Für aktuelle Design-Dokumente der Version 2 muss jede gesicherte Header-Variante ein boolesches `roundedCorners`-Flag besitzen; ältere Dokumente bleiben als migrationsfähige Vorab-Sicherung zulässig. Das Manifest dokumentiert die vollständigen und aktuellen Design-Datensätze sowie die Zahlen der gesicherten Header-Varianten, Ecken-Einstellungen und aktivierten Abrundungen. Dafür dienen `tenant_design_complete_rows`, `tenant_design_current_rows`, `tenant_design_current_complete_rows`, `tenant_header_variants`, `tenant_header_corner_preferences` und `tenant_header_rounded_enabled`.
