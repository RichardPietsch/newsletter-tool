...
export type UiTextKey = Leaves<typeof uiText>;

function lookup(path: string) {
  return path.split('.').reduce<unknown>((current, segment) => (current as Record<string, unknown>)?.[segment], uiText) as string;
}

export function t(key: UiTextKey) {
  return lookup(key);
}

export function flattenUiText(value: unknown = uiText): string[] {
