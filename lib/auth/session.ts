...
export async function validateSession(token: string): Promise<AuthUser | null> {
  ...
  if (!row) return null;
  const stale = Date.now() - row.session.lastSeenAt.getTime() > 10 * 60 * 1000;
  if (stale) await db.update(sessions).set({ lastSeenAt: new Date() }).where(eq(sessions.id, row.session.id));
  return row.user;
}
