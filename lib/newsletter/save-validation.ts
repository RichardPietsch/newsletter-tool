...
export function validateNewsletterForSave(document: NewsletterDocument): NewsletterSaveIssue[] {
  ...
  return result.error.issues.map((issue) => {
    ...
    const topLevelBlock = blockIndex === undefined ? undefined : document.blocks[blockIndex];
    const nestedIndex = issue.path[2] === 'blocks' && typeof issue.path[3] === 'number' ? issue.path[3] : undefined;
    const block =
      topLevelBlock?.type === 'backgroundSection' && nestedIndex !== undefined
        ? topLevelBlock.blocks[nestedIndex]
        : topLevelBlock;
    const itemsPathIndex = issue.path.indexOf('items');
    const itemIndex =
      itemsPathIndex !== -1 && typeof issue.path[itemsPathIndex + 1] === 'number'
        ? (issue.path[itemsPathIndex + 1] as number)
        : undefined;
