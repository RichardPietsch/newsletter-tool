...
export default async function AdminLogsPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  if (search.severity === 'info' || search.severity === 'warning' || search.severity === 'error')
    filters.push(eq(auditEvents.severity, search.severity));
  if (search.from && !Number.isNaN(Date.parse(search.from)))
    filters.push(gte(auditEvents.createdAt, new Date(search.from)));
  if (search.to && !Number.isNaN(Date.parse(search.to)))
    filters.push(lte(auditEvents.createdAt, new Date(`${search.to}T23:59:59.999`)));
  ...
  return (
    <main className="mx-auto max-w-7xl p-6">
      <h1 className="text-3xl font-bold">{t('admin.eventLogs')}</h1>
      <form method="get" className="mt-6 grid gap-3 rounded-xl bg-white p-4 shadow-sm md:grid-cols-5">
        <select name="tenantId" defaultValue={search.tenantId ?? ''} className="rounded border p-2">
        ...
        <input
          ...
          placeholder={t('admin.eventType')}
          className="rounded border p-2"
        />
        <select name="severity" defaultValue={search.severity ?? ''} className="rounded border p-2">
        ...
        <div className="flex gap-2">
          <input name="to" type="date" defaultValue={search.to ?? ''} className="min-w-0 flex-1 rounded border p-2" />
          <button className="rounded bg-blue-700 px-3 text-white">{t('admin.filter')}</button>
        </div>
      </form>
      <div className="mt-6 space-y-2">
        {events.map((event) => (
          <article
            key={event.id}
            className="grid gap-2 rounded border bg-white p-3 text-sm md:grid-cols-[11rem_13rem_6rem_8rem_1fr]"
          >
            <time>{event.createdAt.toLocaleString('de-DE')}</time>
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                {t('admin.correlation')} {event.correlationId}
              </p>
            </div>
          </article>
        ))}
