...
import { TenantDesignEditor } from '@/components/admin/tenant-design-editor';
...
export default async function TenantDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!tenant) notFound();
  const [accounts, events, settings, newsletterRows] = await Promise.all([
    db
      .select()
      .from(users)
      .where(and(eq(users.tenantId, id), eq(users.role, 'tenant_member')))
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-6 p-6">
      <div>
        <a href="/admin" className="text-sm text-blue-700">
          {t('admin.backToTenants')}
        </a>
        <h1 className="mt-2 text-3xl font-bold">{tenant.name}</h1>
      ...
      <section className="rounded-xl bg-white p-5 shadow-sm">
        ...
        <form action={`/api/admin/tenants/${id}`} method="post" className="mt-4 space-y-3">
          ...
          <label className="block text-sm font-medium">
            {t('admin.internalAdminNotes')}
            <textarea
              className="mt-1 min-h-24 w-full rounded border p-2"
              name="adminNotes"
              defaultValue={tenant.adminNotes ?? ''}
              maxLength={2000}
            />
          </label>
        ...
        <div className="mt-5 flex flex-wrap gap-3 border-t pt-5">
          ...
          <form action="/api/admin/support" method="post">
            ...
            <button className="rounded border border-amber-700 px-4 py-2 text-amber-900">
              {t('admin.startSupport')}
            </button>
          </form>
        </div>
      </section>
            ...
            <article key={account.id} className="flex flex-wrap items-center gap-3 py-3">
              ...
              <form action={`/api/admin/tenants/${id}/accounts/${account.id}`} method="post">
                <input
                  type="hidden"
                  name="operation"
                  value={account.status === 'active' ? 'deactivate' : 'reactivate'}
                />
