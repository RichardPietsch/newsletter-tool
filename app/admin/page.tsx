  ...
  return (
    <main className="mx-auto max-w-7xl p-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold">{t('admin.tenantsTitle')}</h1>
      ...
      <section className="mt-6 rounded-xl bg-white p-5 shadow-sm">
        ...
        <form action="/api/admin/tenants" method="post" className="mt-4 grid gap-3 md:grid-cols-[1fr_2fr_auto]">
          ...
          />
          <button className="rounded bg-blue-700 px-4 py-2 text-white">{t('admin.create')}</button>
        </form>
      </section>
      ...
      <div className="mt-6 overflow-x-auto rounded-xl bg-white shadow-sm">
        <table className="w-full text-left text-sm">
          <thead className="border-b bg-slate-50">
            <tr>
              <th className="p-3">{t('admin.tenant')}</th>
              <th>{t('misc.status')}</th>
              <th>{t('admin.created')}</th>
              <th>{t('admin.activeAccounts')}</th>
              <th>{t('admin.lastLogin')}</th>
              <th>{t('admin.lastActivity')}</th>
              <th>{t('admin.recentErrors')}</th>
            </tr>
          </thead>
          <tbody>
            {summaries.map(({ tenant, activeAccounts, lastLoginAt, recentErrors }) => (
              <tr key={tenant.id} className="border-b last:border-0">
                <td className="p-3">
                  <Link className="font-medium text-blue-700" href={`/admin/tenants/${tenant.id}`}>
                    {tenant.name}
                  </Link>
                </td>
                <td>{tenant.status}</td>
