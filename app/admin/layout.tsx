    ...
    <div className="flex min-h-screen flex-col bg-slate-100">
      <header className="border-b bg-slate-950 text-white">
        <div className="mx-auto flex max-w-7xl items-center gap-6 px-6 py-4">
          <Link href="/admin" className="font-semibold">
            {t('admin.platformAdministration')}
          </Link>
          <Link href="/admin/logs" className="text-sm text-slate-200">
            {t('admin.eventLogs')}
          </Link>
          <Link href="/account" className="ml-auto text-sm text-slate-200">
            {t('account.title')}
          </Link>
        </div>
      </header>
      ...
      <footer className="border-t border-slate-200 bg-white text-xs text-slate-500">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-x-2 gap-y-1 px-6 py-3">
          <span>
            {t('admin.appName')} · {t('admin.versionLabel')} {buildInfo.version}
          </span>
