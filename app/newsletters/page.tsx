      ...
      <main className="mx-auto max-w-3xl p-8">
        ...
        <div className="space-y-3">
          {rows.map((newsletter) => (
            <a
              key={newsletter.id}
              data-tour="newsletter-card"
              className="flex items-center justify-between gap-4 rounded border bg-white p-4 transition hover:border-blue-600"
              href={`/newsletters/${newsletter.id}`}
            >
              <span>
                <span className="font-medium">{newsletter.title}</span>
                <span className="block text-sm text-slate-500">
                  {new Date(newsletter.updatedAt).toLocaleString('de-DE')}
                </span>
              </span>
              {newsletter.sentAt ? (
                <span className="inline-flex items-center gap-2 rounded-full bg-green-50 px-3 py-1 text-sm font-medium text-green-700">
                  <MdiIcon name="emailCheck" className="h-5 w-5" /> {t('misc.sent')}
                </span>
              ) : null}
            </a>
          ))}
