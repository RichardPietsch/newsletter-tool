...
export function FeaturedEventBlock({ block }: { block: B }) {
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: isWhite ? styles.cardBackground : styles.navy,
        color: isWhite ? styles.bodyText : '#ffffff',
      }}
    >
      {block.image?.src && (
        <img
      ...
      <div className="flex flex-col px-8 py-7">
        <div
          ...
          style={{ color: isWhite ? styles.red : '#cddde3' }}
        >
          {block.overline}
        </div>
        <h2 className="mt-3 font-serif text-3xl leading-tight">{block.title}</h2>
        {block.date && (
          <p className="mt-5 text-sm" style={{ color: isWhite ? styles.mutedText : '#c7d4da' }}>
            {block.date}
          </p>
        )}
        {block.description && (
          <p className="mt-3 text-sm leading-relaxed" style={{ color: isWhite ? styles.bodyText : '#dbe5e9' }}>
            {block.description}
          </p>
        )}
        {block.buttonUrl && (
          <span
            ...
            style={{ backgroundColor: isWhite ? styles.navy : '#dbe7eb', color: isWhite ? '#ffffff' : styles.navy }}
          >
            {block.buttonLabel}
          </span>
        )}
