...
export function HeaderBlock({ branding, settings, headerVariantId, squareBottom = false }: { branding: string; settings?: GlobalSettings; headerVariantId?: string; squareBottom?: boolean }) {
  const variant = settings?.headerVariants.find((item) => item.id === headerVariantId) ?? settings?.headerVariants[0];

  return (
    <div className={`mt-6 overflow-hidden bg-white px-8 py-5 text-center ${squareBottom ? 'rounded-t-[4px]' : 'rounded-[4px]'}`}>
