...
export function SideRail({ onExport, onOpenNewsletterSettings, onOpenMedia, onOpenSettings, onOpenAccount }: SideRailProps) {
  return (
    <nav className="sticky top-0 z-20 flex h-screen w-20 shrink-0 flex-col items-center gap-3 border-r bg-white py-4" aria-label={t('misc.functionRail')}>
      <RailButton href="/newsletters" label={t('misc.newsletterOverview')}><MdiIcon name="home" /></RailButton>
      <RailButton onClick={onOpenMedia} label={t('misc.mediaTitle')}><MdiIcon name="media" /></RailButton>
      <RailButton onClick={onOpenSettings} label={t('misc.settingsTitle')}><MdiIcon name="cog" /></RailButton>
      <RailButton onClick={onOpenAccount} label={t('account.title')}><MdiIcon name="account" /></RailButton>
      <div className="mt-auto flex flex-col items-center gap-3">
        {onOpenNewsletterSettings ? <RailButton onClick={onOpenNewsletterSettings} label={t('misc.newsletterSettingsTitle')}><MdiIcon name="emailEdit" className="h-7 w-7" /></RailButton> : null}
        {onExport ? <RailButton onClick={onExport} label={t('export.dialogTitle')} emphasized><MdiIcon name="download" className="h-7 w-7" /></RailButton> : null}
      </div>
    </nav>
  );
}
