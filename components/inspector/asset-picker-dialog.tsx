    ...
    <Overlay title={t('image.choose')} onClose={onClose}>
      <div className="space-y-4 p-6">
        ...
        {!loading && assets.length === 0 ? (
          <p className="rounded border border-dashed p-4 text-sm text-slate-600">{t('misc.noMedia')}</p>
        ) : null}
        <div className="grid grid-cols-2 gap-3">
          {assets.map((asset) => (
            <button
              key={asset.id}
              className="overflow-hidden rounded border bg-white text-left hover:border-blue-600"
              onClick={() => {
                onSelect(asset);
                onClose();
              }}
