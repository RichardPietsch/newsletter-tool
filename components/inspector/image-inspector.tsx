...
export function ImageInspector({ block, onChange, validationIssues = [] }: { block: ImageBlock; onChange: (patch: Partial<ImageBlock>) => void; validationIssues?: NewsletterSaveIssue[] }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <button type="button" className="rounded bg-blue-700 px-4 py-2 text-sm text-white" onClick={() => setPickerOpen(true)}>{t('image.choose')}</button>
      {block.src ? <img src={block.src} alt={block.decorative ? '' : block.alt || ''} className="max-h-40 w-full rounded border object-contain" /> : <p className="rounded border border-dashed p-4 text-sm text-slate-600">{t('image.none')}</p>}
      <label className="block text-sm font-medium">{t('image.alt')}{block.decorative ? '' : ' *'}
      ...
      <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={block.decorative} onChange={(event) => onChange({ decorative: event.target.checked, alt: event.target.checked ? '' : block.alt })} /> {t('image.decorative')}</label>
      <label className="block text-sm font-medium">{t('image.optionalTargetUrl')}
